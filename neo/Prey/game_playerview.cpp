...
void hhPlayerView::WriteToSnapshot( idBitMsgDelta &msg ) const {
	...
	if (viewOverlayMaterial) {
		msg.WriteInt(gameLocal.ServerRemapDecl(-1, DECL_MATERIAL, viewOverlayMaterial->Index()));
	}
	else {
		msg.WriteInt(-1);
	}
	...
	for (int i = 0; i < MAX_SCREEN_BLOBS_SYNC; i++) {
		...
		*/
		...
		if (blob->material) {
			msg.WriteInt(gameLocal.ServerRemapDecl(-1, DECL_MATERIAL, blob->material->Index()));
		}
		else {
			msg.WriteInt(-1);
		}
...
void hhPlayerView::RenderPlayerView( idUserInterface *hud ) {
	...
	if ( net_clientLagOMeter.GetBool() && lagoMaterial && gameLocal.isClient ) {
		...
		if ( cvarSystem->GetCVarBool( "gui_hudAdjustAspect" ) ) {
			// similar to ANCHOR_BOTTOM_LEFT
