...
tbool genTangSpace(const SMikkTSpaceContext * pContext, const float fAngularThreshold)
{
	...
	piTriListIn = (int *)pContext->m_pInterface->m_alloc(sizeof(int)*3*iNrTrianglesIn);
	pTriInfos = (STriInfo *)pContext->m_pInterface->m_alloc(sizeof(STriInfo)*iNrTrianglesIn);
	if (piTriListIn==NULL || pTriInfos==NULL)
	{
		if (piTriListIn!=NULL) pContext->m_pInterface->m_free(piTriListIn);
		if (pTriInfos!=NULL) pContext->m_pInterface->m_free(pTriInfos);
		return TFALSE;
	}
	...
	pGroups = (SGroup *) pContext->m_pInterface->m_alloc(sizeof(SGroup)*iNrMaxGroups);
	piGroupTrianglesBuffer = (int *) pContext->m_pInterface->m_alloc(sizeof(int)*iNrTrianglesIn*3);
	if (pGroups==NULL || piGroupTrianglesBuffer==NULL)
	{
		if (pGroups!=NULL) pContext->m_pInterface->m_free(pGroups);
		if (piGroupTrianglesBuffer!=NULL) pContext->m_pInterface->m_free(piGroupTrianglesBuffer);
		pContext->m_pInterface->m_free(piTriListIn);
		pContext->m_pInterface->m_free(pTriInfos);
		return TFALSE;
	}
	...
	psTspace = (STSpace *) pContext->m_pInterface->m_alloc(sizeof(STSpace)*iNrTSPaces);
	if (psTspace==NULL)
	{
		pContext->m_pInterface->m_free(piTriListIn);
		pContext->m_pInterface->m_free(pTriInfos);
		pContext->m_pInterface->m_free(pGroups);
		pContext->m_pInterface->m_free(piGroupTrianglesBuffer);
		return TFALSE;
	}
	...
	if (!bRes)	// if an allocation in GenerateTSpaces() failed
	{
		...
		pContext->m_pInterface->m_free(pTriInfos); pContext->m_pInterface->m_free(piTriListIn); pContext->m_pInterface->m_free(psTspace);
		return TFALSE;
	}
	...
	pContext->m_pInterface->m_free(pTriInfos); pContext->m_pInterface->m_free(piTriListIn);
	...
	pContext->m_pInterface->m_free(psTspace);
...
static void GenerateSharedVerticesIndexList(int piTriList_in_and_out[], const SMikkTSpaceContext * pContext, const int iNrTrianglesIn)
{
	...
	// make allocations
	piHashTable = (int *) pContext->m_pInterface->m_alloc(sizeof(int)*iNrTrianglesIn*3);
	piHashCount = (int *) pContext->m_pInterface->m_alloc(sizeof(int)*g_iCells);
	piHashOffsets = (int *) pContext->m_pInterface->m_alloc(sizeof(int)*g_iCells);
	piHashCount2 = (int *) pContext->m_pInterface->m_alloc(sizeof(int)*g_iCells);

	if (piHashTable==NULL || piHashCount==NULL || piHashOffsets==NULL || piHashCount2==NULL)
	{
		if (piHashTable!=NULL) pContext->m_pInterface->m_free(piHashTable);
		if (piHashCount!=NULL) pContext->m_pInterface->m_free(piHashCount);
		if (piHashOffsets!=NULL) pContext->m_pInterface->m_free(piHashOffsets);
		if (piHashCount2!=NULL) pContext->m_pInterface->m_free(piHashCount2);
	...
	pContext->m_pInterface->m_free(piHashCount2);
	...
	pTmpVert = (STmpVert *) pContext->m_pInterface->m_alloc(sizeof(STmpVert)*iMaxCount);
	...
	if (pTmpVert!=NULL) { pContext->m_pInterface->m_free(pTmpVert); }
	pContext->m_pInterface->m_free(piHashTable);
	pContext->m_pInterface->m_free(piHashCount);
	pContext->m_pInterface->m_free(piHashOffsets);
}
...
static void MergeVertsFast(int piTriList_in_and_out[], STmpVert pTmpVert[], const SMikkTSpaceContext * pContext, const int iL_in, const int iR_in)
{
	...
	// stop if all vertices are NaNs or infinities
	if (fSep != fSep || fSep > 1e30f || fSep < -1e30f) // NaN check and infinity guard
		return;
...
static void InitTriInfo(STriInfo pTriInfos[], const int piTriListIn[], const SMikkTSpaceContext * pContext, const int iNrTrianglesIn)
{
	...
	// match up edge pairs
	{
		SEdge * pEdges = (SEdge *) pContext->m_pInterface->m_alloc(sizeof(SEdge)*iNrTrianglesIn*3);
		if (pEdges==NULL)
		...
		else
		{
			...
			pContext->m_pInterface->m_free(pEdges);
		}
	}
}
                             ...
                             const SMikkTSpaceContext * pContext)
{
	...
	pSubGroupTspace = (STSpace *) pContext->m_pInterface->m_alloc(sizeof(STSpace)*iMaxNrFaces);
	pUniSubGroups = (SSubGroup *) pContext->m_pInterface->m_alloc(sizeof(SSubGroup)*iMaxNrFaces);
	pTmpMembers = (int *) pContext->m_pInterface->m_alloc(sizeof(int)*iMaxNrFaces);
	if (pSubGroupTspace==NULL || pUniSubGroups==NULL || pTmpMembers==NULL)
	{
		if (pSubGroupTspace!=NULL) pContext->m_pInterface->m_free(pSubGroupTspace);
		if (pUniSubGroups!=NULL) pContext->m_pInterface->m_free(pUniSubGroups);
		if (pTmpMembers!=NULL) pContext->m_pInterface->m_free(pTmpMembers);
		return TFALSE;
	}
	...
	for (g=0; g<iNrActiveGroups; g++)
	{
		...
		for (i=0; i<pGroup->iNrFaces; i++)	// triangles
		{
			...
			{
				// insert new subgroup
				int * pIndices = (int *) pContext->m_pInterface->m_alloc(sizeof(int)*iMembers);
				if (pIndices==NULL)
				{
					// clean up and return false
					int s=0;
					for (s=0; s<iUniqueSubGroups; s++)
						pContext->m_pInterface->m_free(pUniSubGroups[s].pTriMembers);
					pContext->m_pInterface->m_free(pUniSubGroups);
					pContext->m_pInterface->m_free(pTmpMembers);
					pContext->m_pInterface->m_free(pSubGroupTspace);
					return TFALSE;
				}
