...
bool MaterialPreviewView::Draw( const ImVec2& size )
{
	if( ImGui::BeginChild( "MaterialPreviewView", size, ImGuiChildFlags_Borders ) )
	{
		...
		ImGui::InvisibleButton(
			"###MaterialPreviewRenderedView", size, ImGuiButtonFlags_AllowOverlap | ImGuiButtonFlags_MouseButtonLeft | ImGuiButtonFlags_MouseButtonMiddle | ImGuiButtonFlags_MouseButtonRight );
...
void idGLDrawableView::UpdateLights( void )
{
	...
	for( i = 0; i < viewLights.Num(); i++ )
	{
		...
		vLight->renderLight.lightRadius[0] = vLight->renderLight.lightRadius[1] = vLight->renderLight.lightRadius[2] = vLight->radius;
